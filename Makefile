...
lint: ## static code analysis
	black --line-length 120 --check fixstrap tests
	flake8 fixstrap
	pylint fixstrap
	mypy --python-version 3.9 --strict --install-types --non-interactive fixstrap tests
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source fixstrap -m pytest
	coverage combine
	coverage report -m
	coverage html
